...
import { Container, Row, Col, Card, Button, Spinner, Image, Modal, Badge } from 'react-bootstrap';
...
const Profile = () => {
    ...
    // useEffect(() => {
    //     // If current user is ADMIN and visiting their own profile, redirect to dashboard
    //     if (currentUser && currentUser.role === 'ADMIN' && (!userId || userId === currentUser.userId.toString())) {
    ...
    useEffect(() => {
        if (currentUser && profileUser && currentUser.sub === profileUser.username) {
    ...
    const fetchProfileData = async () => {
        ...
        try {
            ...
            // 3. Get Follow Counts
            const followersRes = await api.get(`/users/${targetUserId}/followers/count`);
            setFollowersCount(followersRes.data);

            const followingRes = await api.get(`/users/${targetUserId}/following/count`);
            setFollowingCount(followingRes.data);

            // 4. Get Applications (only if it's my profile)
            if (currentUser && currentUser.userId.toString() === targetUserId.toString()) {
            ...
            if (currentUser && currentUser.userId.toString() !== targetUserId.toString()) {
               const isFollowingRes = await api.get(`/users/${targetUserId}/is-following`);
    ...
    const getRelationshipButton = (user) => {
        ...
        if (iFollowThem && theyFollowMe) {
            ...
            return (
                <Button variant="outline-secondary" size="sm" className="rounded-pill px-3" onClick={() => handleModalUserFollow(user)}>
                    Following
                </Button>
            );
        } else if (theyFollowMe) {
            return (
                <Button variant="primary" size="sm" className="rounded-pill px-3" onClick={() => handleModalUserFollow(user)}>
                    Follow Back
                </Button>
            );
        } else {
            return (
                 <Button variant="outline-primary" size="sm" className="rounded-pill px-3" onClick={() => handleModalUserFollow(user)}>
                    Follow
                </Button>
            );
        }
    ...
    if (!profileUser) return <div className="text-center mt-5">User not found</div>;

    const isMyProfile = currentUser && currentUser.userId.toString() === targetUserId.toString();
        ...
        <Card className="border-0 shadow-sm h-100 rounded-4 transition hover-shadow">
            <Card.Body className="p-3">
                <div className="d-flex align-items-center justify-content-between">
                    <div>
                        <p className="text-secondary small fw-bold mb-1 text-uppercase tracking-wider" style={{ fontSize: '0.65rem' }}>{title}</p>
        ...
        <Container className="py-5">
                    ...
                    <Card.Body className="p-4">
                        <div className="d-flex flex-column flex-md-row align-items-center gap-4">
                            ...
                            <div className="text-center text-md-start flex-grow-1 pt-2">
                                <h2 className="fw-black mb-1" style={{ color: '#1e293b' }}>{profileUser.fullName || profileUser.username}</h2>
                                <p className="text-muted mb-3 fw-medium">@{profileUser.username}</p>
                                ...
                                <div className="d-flex flex-wrap gap-2 justify-content-center justify-content-md-start">
                                    ...
                                    {isMyProfile && (
                                        <div className="d-flex gap-2">
                                            ...
                                            >
                                                <i className={`bi bi-bell-fill fs-5 ${hasUnread ? 'text-danger' : 'text-primary'}`}></i>
                                            </div>
                                        </div>
                                    )}
                                </div>
                        ...
                        <Row className="g-3 mt-2">
                            <Col xs={6} md={isMyProfile ? 3 : 4}>
                                <div onClick={() => {}} style={{ cursor: 'default' }}>
                                    <StatCard title="Posts" value={jobs.length} icon="grid" color="primary" />
                                </div>
                            </Col>
                            <Col xs={6} md={isMyProfile ? 3 : 4}>
                                <div onClick={openFollowersModal} style={{ cursor: 'pointer' }}>
                                    <StatCard title="Followers" value={followersCount} icon="people" color="success" />
                                </div>
                            </Col>
                            <Col xs={6} md={isMyProfile ? 3 : 4}>
                                <div onClick={openFollowingModal} style={{ cursor: 'pointer' }}>
                                    <StatCard title="Following" value={followingCount} icon="person-plus" color="info" />
                                </div>
                            </Col>
                            {isMyProfile && (
                                <Col xs={6} md={3}>
                                    <Link to="/applications" className="text-decoration-none">
                                        <StatCard title="Applications" value={applications.length} icon="file-earmark-text" color="warning" />
                                    </Link>
                                </Col>
                            )}
                        </Row>
            ...
            {jobs.length === 0 ? (
                <div className="text-center py-5 bg-white rounded-4 shadow-sm border-0">
                    <div className="mb-4">
                        <i className="bi bi-grid-3x3-gap" style={{ fontSize: '4rem', color: '#cbd5e1' }}></i>
                    </div>
                            ...
                            <Card className="h-100 shadow-sm border-0 rounded-4 transition hover-shadow overflow-hidden">
                                <Card.Body className="p-4 d-flex flex-column">
                                    ...
                                    <div className="mt-auto pt-3 border-top d-flex justify-content-between align-items-center">
                                        ...
                                        <Link to={`/search`} className="btn btn-sm btn-outline-primary rounded-pill px-3 fw-bold">
                                            View
                                        </Link>
                                    </div>
                                </Card.Body>
                    ...
                    {modalLoading ? (
                        ...
                        modalUsers.length === 0 ? (
                            ...
                            <div className="list-group list-group-flush">
                                {modalUsers.map(user => (
                                    <div key={user.id} className="list-group-item border-0 d-flex align-items-center gap-3 py-3 px-4 hover-bg-light transition">
                                         <Link 
                                            to={`/u/${user.id}`}
                                        ...
                                        <div className="flex-grow-1 overflow-hidden">
                                            <Link to={`/u/${user.id}`} className="text-decoration-none text-dark" onClick={handleCloseModal}>
            ...
            <NotificationDrawer show={showNotifications} handleClose={() => setShowNotifications(false)} />
        </Container>
    );
};
